...
6. **Every meaningful mutation the API already authorizes is instrumented.** One
   ...
   | `POST /projects/:projectId/deploy` (manual only) | `deploy.triggered` | user (issue #26) |
...
7. **Explicitly out of scope, with reasons** — none of these are silently skipped:
   ...
   | Push-driven `deploy` (`handlePushEvent`, the `pull_request`-merge path) | It fires on a push to the default branch, so it has no actor to attribute and would add a row per push to `main`. (Historical note: this row used to cover the *manual* `POST /projects/:id/deploy` trigger too, and recorded the resulting asymmetry with `POST /projects/:id/rollback` as an open follow-up. Issue #26 closed it — both operator-initiated actions, `deploy.triggered` and `deploy.rolled_back`, are audited by [ADR 022](022-deployment-rollback.md) §8 — leaving only the actorless push path out.) |
