...
| **Model configuration** | The `MODEL_BASE_URL`/`MODEL_API_KEY`/`MODEL_ID` bundle Pi uses as its OpenAI-chat-completions-compatible backend at the pod-env-var level. Not "agent configuration": Pi itself is fixed (ADR 004); only the model it talks to varies. Since ADR 018, resolved per job kind from a Provider/Model catalog rather than one flat org-wide triplet — see **Provider**, **Model catalog**, **Job model default** below. A project may still supply its own custom triplet directly (bypassing the catalog) as an override. |
| **Account default (model configuration)** | Historical ADR 007 concept: a user's personal fallback model configuration in `user_secrets`. **Superseded and retired by ADR 016** in favor of an Organization-level default; `user_secrets` itself dropped in ADR 018. |
...
| **Organization secrets** | New concept (ADR 016, built): a generic key/value store at the Organization level (`organization_secrets`), inherited automatically by every project under it — mirrors `project_secrets`'s shape. A project-level key of the same name shadows the org-level one. Model config (`MODEL_BASE_URL`/`MODEL_API_KEY`/`MODEL_ID`) no longer lives here at the org level as of ADR 018 — see **Provider**/**Model catalog** — though a project may still set its own custom triplet directly in `project_secrets`. |
