...
| **Organization** *(decided and implemented, ADR 016)* | Entity above Project, replacing `owner_user_id` ownership (ADR 002). A user can belong to several; every user gets a personal one auto-created at signup. See `docs/CONTEXT.md`. |
| **Membership / Role** *(decided and implemented, ADR 016)* | Five org-wide roles (Admin/Developer/Designer/Product Manager/Tester), one per membership, applying across every project in that org. Capability grants are adjustable seed data. |
